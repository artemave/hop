...
def test_derive_session_root_returns_the_provided_directory(tmp_path: Path) -> None:
    session_root = tmp_path / "demo-project" / "app" / "models"
    session_root.mkdir(parents=True)

    assert derive_session_root(session_root) == session_root


def test_derive_session_name_uses_project_directory_basename(tmp_path: Path) -> None:
    session_root = tmp_path / "demo-project"
    session_root.mkdir()

    assert derive_session_name(session_root) == "demo-project"
...
def test_derive_workspace_name_uses_directory_basename(tmp_path: Path) -> None:
    ...
    assert derive_workspace_name(session_root) == "s:demo-project"
...
def test_resolve_project_session_builds_complete_session_identity(tmp_path: Path) -> None:
    ...
    assert resolve_project_session(session_root).session_root == session_root
    assert resolve_project_session(session_root).session_name == "pkg"
    assert resolve_project_session(session_root).workspace_name == "s:pkg"


def test_resolve_project_session_treats_nested_directories_as_distinct_sessions(tmp_path: Path) -> None:
    session_root = tmp_path / "demo-project"
    nested_directory = session_root / "pkg"
    session_root.mkdir()
    nested_directory.mkdir()

    assert resolve_project_session(session_root).session_name == "demo-project"
