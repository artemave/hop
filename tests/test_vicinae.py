...
def test_focused_session_emits_window_kill_and_other_session_switch_scripts() -> None:
    sessions = (
        SessionListing(name="rails", workspace="s:rails", session_root=Path("/projects/rails")),
        SessionListing(name="other", workspace="s:other", session_root=Path("/projects/other")),
    )
...
def test_custom_layout_roles_get_their_own_window_scripts() -> None:
    sessions = (SessionListing(name="rails", workspace="s:rails", session_root=Path("/projects/rails")),)
...
def test_off_session_workspace_emits_only_session_switch_scripts() -> None:
    sessions = (
        SessionListing(name="rails", workspace="s:rails", session_root=Path("/projects/rails")),
        SessionListing(name="other", workspace="s:other", session_root=Path("/projects/other")),
        SessionListing(name="third", workspace="s:third", session_root=Path("/projects/third")),
    )
...
def test_focused_workspace_with_unregistered_session_falls_back_to_off_session_set() -> None:
    sessions = (SessionListing(name="other", workspace="s:other", session_root=Path("/projects/other")),)

    scripts = _targets(
        "s:not-a-real-session",
        sessions,
        windows_for=lambda _: _builtin_windows(),
    )
...
def test_session_without_session_root_does_not_emit_window_scripts() -> None:
    sessions = (
        SessionListing(name="lost", workspace="s:lost", session_root=None),
        SessionListing(name="other", workspace="s:other", session_root=Path("/projects/other")),
    )
...
def test_every_generated_script_advertises_the_hop_icon_with_a_resolvable_path() -> None:
    sessions = (
        SessionListing(name="rails", workspace="s:rails", session_root=Path("/tmp/rails")),
        SessionListing(name="other", workspace="s:other", session_root=Path("/tmp/other")),
    )
...
def test_generated_script_has_directive_header_and_atomic_chmod_markers() -> None:
    sessions = (SessionListing(name="rails", workspace="s:rails", session_root=Path("/tmp/rails")),)
...
def test_window_script_packagename_is_session_name_for_subtitle_context() -> None:
    sessions = (SessionListing(name="rails-app", workspace="s:rails-app", session_root=Path("/tmp/rails")),)
...
def test_switch_script_has_empty_packagename_to_suppress_default_subtitle() -> None:
    ...
    scripts = _targets("s:rails", sessions, windows_for=lambda _: ())
...
def test_remote_session_scripts_use_hop_remote_env_not_cd() -> None:
    ...
    sessions = (
        SessionListing(
            ...
            workspace="s:thonon-les-pains",
            session_root=Path("/home/admin/projects/thonon-les-pains"),
            host="devbox.local",
        ),
    )

    scripts = _targets("s:thonon-les-pains", sessions, windows_for=lambda _: _windows(("editor", "nvim")))
    by_filename = {s.filename: s.content for s in scripts}
...
def test_switch_script_dispatches_hop_switch_with_quoted_session_name() -> None:
    sessions = (
        SessionListing(name="rails", workspace="s:rails", session_root=Path("/tmp/rails")),
        SessionListing(name="weird name", workspace="s:weird name", session_root=Path("/tmp/weird")),
    )
...
def test_regenerate_wires_focused_workspace_sessions_and_windows_resolver(tmp_path: Path) -> None:
    sway = StubSway(focused_workspace="s:rails")
    sessions = (SessionListing(name="rails", workspace="s:rails", session_root=Path("/tmp/rails")),)
...
def test_switch_script_exports_the_sway_socket_before_dispatching() -> None:
    ...
    sessions = (SessionListing(name="other", workspace="s:other", session_root=Path("/tmp/other")),)
