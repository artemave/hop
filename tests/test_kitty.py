...
def test_get_focused_window_cwd_returns_cwd_of_focused_window() -> None:
    from hop.kitty import get_focused_window_cwd

    factory = StubKittyFactory([_focused_window_payload(cwd="/home/me/projects/foo/src")])

    cwd = get_focused_window_cwd("demo", transport_factory=factory)

    assert cwd == Path("/home/me/projects/foo/src")
    assert factory.calls == [(SESSION_SOCKET, "ls", {"output_format": "json"})]
