...
The session root is exactly the caller's current working directory when `hop` starts. `hop` must not walk ancestors looking for `.git`, `pyproject.toml`, or any other marker file.
...
- **A remote host** (an ssh host with no prefix): the implicit shell is `exec "${XDG_CACHE_HOME:-$HOME/.cache}/hop/kitten" run-shell` — no PATH lookup, no degrade. hop owns this one: `hop ssh` installs that exact path itself and fails loudly if it can't, so by window-launch time the binary is either present or the user has already been told why not. A PATH lookup would only reintroduce the question of *which* kitten runs, which is what the pinned install exists to settle.

For a remote host, `hop ssh` has the remote download the `kitten` matching the host's kitty version (`kitty --version` supplies the GitHub release tag) built for the remote's own OS/arch, into `~/.cache/hop/kitten`. The fetch is skipped when the cached binary already reports that version, is verified by running `run-shell --help` before being moved into place, and never crosses the host's uplink. An explicit `[windows.shell].command` overrides the implicit shell on any backend.
...
For file targets resolved by the open-selection kitten, hop consults the focused session's backend through `hop.focused.paths_exist`: relative candidates are resolved against the same base cwd a click on them dispatches with (`hop.focused.selection_base_cwd`: the backend's probed `workspace_path` when it has one, else the focused window's cwd from kitty's per-session socket, else the session root), then `backend.paths_exist` runs a single shell loop inside the backend (wrapping `<noninteractive_prefix> sh -c '<while-read>'`) and reports which paths exist. The kitten itself owns no session, backend, or IPC awareness — it just calls `paths_exist` and yields marks for the survivors.
...
- links are `hop://<action>/<argument>` URLs, the argument percent-encoded except for `/` and `:`; the only action is `open`, whose argument is the selection (`hop://open/app/models/user.rb:42`). The watcher installs itself as the window's `open_url_handler`: a click on a `hop://` link runs its action — `open` runs the hints kitten's dispatch. A click on a plain `http(s)` URL runs `open` too, so a URL kitty detected in the text itself (clicked before the line's lookup painted its `hop://` link) or a program's own OSC 8 link reaches the session browser with the backend's localhost translation; kitty's `allow_hyperlinks` confirmation doesn't apply to it. Every other URL falls through to kitty
...
Each session's kitty binds `[keys].paste` (a kitty key spec or list; default `ctrl+v` and `ctrl+shift+v`, an empty value disables) to a bundled kitten, injected as a `--override map <key> kitten <path>` at bootstrap — no `kitty.conf` entry required. The kitten runs in the kitty boss thread, so the only clipboard call it makes there is kitty's in-process **MIME-list** check (safe, non-blocking); the image bytes are never read on the boss (a boss-side `Clipboard.get_mime` data read is a silent no-op outside kitty's OSC-52 server path, and a `wl-paste` subprocess on the boss self-deadlocks when kitty owns the selection). Then:

- **image on the clipboard** — the kitten launches a kitty-supervised background process (`boss.run_background_process`, reaped by `ChildMonitor`) running `hop __paste-image <session> <write-path> <mime>`. That helper reads the image with `wl-paste` (a subprocess — blocking it can't freeze the boss, which has already returned) and writes it into the session backend's filesystem namespace via `backend.write_file` (a local write for a host session; a `base64` payload through `<noninteractive_prefix> sh` for a container/remote one) at `/tmp/hop-paste-<ns>.png`, bounded by a timeout so a dead ssh master surfaces as a failure. On exit 0 the kitten pastes that path with `send-text --bracketed-paste=auto`; on a non-zero exit (reported via `notify_on_death`) it writes a failure line into the window instead. Claude Code / Codex attach the file; a shell or editor receives the path.
- **no image on the clipboard, or no focused hop session** — falls through to kitty's native `paste_from_clipboard`.
