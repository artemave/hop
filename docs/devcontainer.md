...
- A startup plugin (dashboard, intro screen, or auto-opened file picker) is intercepting the cmdline before nvim processes hop's keystrokes. Try opening a file via `hop open <path>` after nvim has fully loaded — if it works then but not at first launch, that's the smoking gun.
